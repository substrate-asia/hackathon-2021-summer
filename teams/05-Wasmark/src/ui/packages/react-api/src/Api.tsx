...
import { InitialParamsContext } from '../../apps/src/core';
...
async function loadOnReady (api: ApiPromise, injectedPromise: Promise<InjectedExtension[]>, store: KeyringStore | undefined, types: Record<string, Record<string, string>>): Promise<ApiState> {
  ...
  const { properties, systemChain, systemChainType, systemName, systemVersion } = await retrieve(api, injectedPromise);
  ...
  console.log('isKeyringLoaded()', isKeyringLoaded(), isEthereum)
...
function Api ({ children, store }: Props): React.ReactElement<Props> | null {
  ...
  const value = useMemo<ApiProps>(
    () => ({ ...state, api, apiError, apiUrl: initialTestParams?.endpoint, extensions, isApiConnected, isApiInitialized, isWaitingInjected: !extensions }),
    [apiError, extensions, isApiConnected, isApiInitialized, state, initialTestParams?.endpoint]
  );

  // initial initialization
  useEffect((): void => {
    const provider = new WsProvider(initialTestParams?.endpoint);
    // const signer = new ApiSigner(registry, queuePayload, queueSetTxStatus);
    const types = getDevTypes();

    api = new ApiPromise({ provider, registry, types, typesBundle, typesChain });
    ...
    api.on('ready', (): void => {
      ...
      loadOnReady(api, Promise.resolve([]), store, types)
        .then(setState)
        .catch((error): void => {
          console.error(error);
