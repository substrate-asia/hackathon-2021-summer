 ...
 * 返回不同币种交易所现货价格的最大价差
 */
function getSpotPriceDifference() {
    var spotPriceDifferenceData;
    $.ajax({
        type: "GET",
        url: baseUrl + "/analiysis/spotpricedifference",
        cache: false, //禁用缓存
        data: {
        }, //传入组装的参数
        dataType: "json",
        success: function (result) {
            spotPriceDifferenceData = result;
            drawTable();
        }
    });
    ...
    function drawTable() {
        var str = "";
        spotPriceDifferenceData.forEach((item, index) => {
            str += '<tr>';
            str += '<td>';
        ...
        $("#spotpricedifference-tbody").append(str);
